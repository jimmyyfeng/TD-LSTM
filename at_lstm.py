...
from utils import load_w2v, batch_index, load_inputs_twitter, load_word_id_mapping, load_aspect2id, load_inputs_twitter_at
...
tf.app.flags.DEFINE_string('train_file_path', 'data/restaurant/rest_2014_lstm_train.txt', 'training file')
tf.app.flags.DEFINE_string('validate_file_path', 'data/restaurant/rest_2014_lstm_test.txt', 'validating file')
tf.app.flags.DEFINE_string('test_file_path', 'data/restaurant/rest_2014_lstm_test.txt', 'testing file')
tf.app.flags.DEFINE_string('embedding_file_path', 'data/restaurant/rest_2014_word_embedding.txt', 'embedding file')
...
class LSTM(object):
    ...
    def __init__(self, embedding_dim=100, batch_size=64, n_hidden=100, learning_rate=0.01,
        ...
        self.aspect_id_mapping = load_aspect2id(FLAGS.aspect_id_file_path)
        self.aspect_embedding = tf.Variable(
            tf.random_uniform([len(self.aspect_id_mapping), self.embedding_dim], -1.0, 1.0),
    ...
    def dynamic_lstm(self, inputs, target):
        """
        ...
        target = tf.zeros(tf.shape(inputs), dtype=tf.float32) + target
        ...
        with tf.name_scope('dynamic_rnn'):
            ...
            output = tf.gather(tf.reshape(outputs, [-1, self.n_hidden]), index)  # ba
    ...
    def run(self):
        ...
        prob = self.dynamic_lstm(inputs, aspect)
        ...
        with tf.Session() as sess:
            ...
            for i in xrange(self.n_iter):
                for train, _ in self.get_batch_data(tr_x, tr_sen_len, tr_y, tr_target_word, self.batch_size, 0.5):
                    _, step, summary = sess.run([optimizer, global_step, train_summary_op], feed_dict=train)
                    train_summary_writer.add_summary(summary, step)
                acc, loss, cnt = 0., 0., 0
                for test, num in self.get_batch_data(te_x, te_sen_len, te_y, te_target_word, 2000, 1.0):
    ...
    def get_batch_data(self, x, sen_len, y, target_words, batch_size, keep_prob):
